...
class MainWindow:
    ...
    def init_main_window(self):
        ...
        # 左上角（占用情况显示）
        self.frame_left_top = tk.LabelFrame(self.frame_left, text="资源占用", borderwidth=2, relief="groove", width=200, height=100)
        self.frame_left_top.pack(side="top", expand=False, fill="both", padx=10, pady=10)
        #  显示CPU、内存、网速情况
        self.cpu_label = tk.Label(self.frame_left_top, text="CPU: 0%", width=10, height=1, pady=10)
        self.cpu_label.pack(side="left", expand=True, fill="both")
        self.memory_label = tk.Label(self.frame_left_top, text="内存: 0%", width=10, height=1)
        self.memory_label.pack(side="left", expand=True, fill="both")
        self.network_label = tk.Label(self.frame_left_top, text="网速: 0KB/s", width=10, height=1)
        self.network_label.pack(side="left", expand=True, fill="both")

        # 左下角（群聊消息显示）
        self.frame_left_bottom = tk.LabelFrame(self.frame_left, text="QQ消息", borderwidth=2, relief="groove", width=200, height=200)
        self.frame_left_bottom.pack(side="bottom", expand=True, fill="both", padx=10, pady=10)
        # 添加滚动条和消息显示框
        self.msg_scrollbar = tk.Scrollbar(self.frame_left_bottom)
        self.msg_scrollbar.pack(side="right", fill="y")
        self.msg_text = tk.Text(self.frame_left_bottom, yscrollcommand=self.msg_scrollbar.set, width=1, height=1, state="disabled")
        self.msg_text.pack(side="left", expand=True, fill="both")
        self.msg_scrollbar.config(command=self.msg_text.yview)

        # 右上角（机器人指令输出显示）
        self.frame_right_top = tk.LabelFrame(self.frame_right, text="机器人日志", borderwidth=2, relief="groove", width=200, height=200)
        self.frame_right_top.pack(side="top", expand=True, fill="both", padx=10, pady=10)
        # 添加滚动条和消息显示框
        self.log_scrollbar = tk.Scrollbar(self.frame_right_top)
        self.log_scrollbar.pack(side="right", fill="y")
        self.log_text = tk.Text(self.frame_right_top, yscrollcommand=self.log_scrollbar.set, width=1, height=1, state="disabled")
        self.log_text.pack(side="left", expand=True, fill="both")
        self.log_scrollbar.config(command=self.log_text.yview)

        # 右下角（机器人指令输入）
        self.frame_right_bottom = tk.LabelFrame(self.frame_right, text="发送指令", borderwidth=2, relief="groove", width=200, height=50)
        self.frame_right_bottom.pack(side="bottom", expand=False, fill="both", padx=10, pady=10)
        # 添加输入框和发送按钮
        ...
        self.send_button = tk.Button(self.frame_right_bottom, text="发送", command=self.send_message)
    ...
    def update_resource_usage(self):
        ...
        # 计算网速
        network_speed = (current_bytes_sent - self.last_bytes_sent + current_bytes_recv - self.last_bytes_recv) / 1024
        ...
        if network_speed > 1024 * 1024:
            ...
            self.network_label.config(text="网速: " + str(int((network_speed) / 1024)) + "MB/s")
        else:
